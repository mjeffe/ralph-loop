...
1. **Review configuration:**
   ...
   vim .ralph/config
   ```
   Set `AGENT` to your coding agent (`amp`, `claude`, `cline`, `codex`, `pi`).
   For `pi`, set `PI_MODEL` (e.g. an OpenRouter model) and export `OPENROUTER_API_KEY`.
...
│   ├── agents/                     # Agent scripts (amp.sh, claude.sh, cline.sh, codex.sh, pi.sh)

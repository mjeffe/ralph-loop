...
Hardcode these overrides in the entrypoint's first-boot `.env` creation
(sed commands after `cp .env.example .env`) to simplify the single-container setup:
...
3. If `.env.testing` specifies a separate test database (e.g., `DB_DATABASE=myapp_testing`),

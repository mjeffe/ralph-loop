...
1. **Read inputs** — Study `AGENTS.md`, `${SPECS_DIR}/README.md`, and all target-state specs in `${SPECS_DIR}/`. **Do NOT read or generate tasks from process specs** — process specs are handled separately by `ralph plan --process`. The configured process-specs directory for this project is `${PROCESS_DIR}` (empty value means none configured; no exclusion needed). The prohibition is categorical: even if `${SPECS_DIR}/README.md` indexes process specs, skip those entries — do not follow the references and do not generate tasks from them. If `${RALPH_HOME}/implementation_plan.md` exists, read it to understand prior progress.
